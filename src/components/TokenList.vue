<template>
	<div class="token-list-modal">
			...
			<div class="modal-content" @click.stop="handleModalContentClick">
				<div class="modal-header">
					...
					<div class="header-actions">
						...
						<!-- 同步按钮 - 仅双向存储模式显示 -->
						<button v-if="isDatabaseAvailable" @click="handleBidirectionalSync" class="btn info small" :disabled="isSyncing" :title="$t('tokenList.syncTooltip')">
					...
					<div v-else class="token-list">
						...
						<div class="status-stats-bar">
							<div class="status-stats-container">
								<!-- 全部 -->
								<div :class="['status-stat-card', { selected: selectedStatusFilter === null }]" @click="selectStatusFilter(null)" :title="$t('tokenList.allStatus')">
								...
								<!-- 活跃 -->
								<div :class="['status-stat-card', 'active', { selected: selectedStatusFilter === 'ACTIVE' }]" @click="selectStatusFilter('ACTIVE')" :title="$t('tokenList.activeStatus')">
								...
								<!-- 封禁 -->
								<div :class="['status-stat-card', 'suspended', { selected: selectedStatusFilter === 'SUSPENDED' }]" @click="selectStatusFilter('SUSPENDED')" :title="$t('tokenList.suspendedStatus')">
								...
								<!-- 其他 -->
								<div :class="['status-stat-card', 'other', { selected: selectedStatusFilter === 'OTHER' }]" @click="selectStatusFilter('OTHER')" :title="$t('tokenList.otherStatus')">
						...
						<div class="balance-filter-bar">
							<div class="balance-filter-container">
								<!-- 全部余额 -->
								<div :class="['balance-stat-card', { selected: selectedBalanceFilter === null }]" @click="selectBalanceFilter(null)" :title="$t('tokenList.allBalance')">
									<div class="stat-icon all">
										<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
									...
									<div class="stat-content">
										<div class="stat-label">{{ $t('tokenList.allBalance') }}</div>
										<div class="stat-value">{{ balanceStatistics.TOTAL }}</div>
									</div>
								</div>

								<!-- 余额>0 -->
								<div :class="['balance-stat-card', 'nonzero', { selected: selectedBalanceFilter === 'nonzero' }]" @click="selectBalanceFilter('nonzero')" :title="$t('tokenList.balanceNonzero')">
									<div class="stat-icon nonzero">
										<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
											<path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z" />
										</svg>
									</div>
									<div class="stat-content">
										<div class="stat-label">{{ $t('tokenList.balanceNonzero') }}</div>
										<div class="stat-value">{{ balanceStatistics.NONZERO }}</div>
									</div>
								</div>

								<!-- 余额为0 -->
								<div :class="['balance-stat-card', 'zero', { selected: selectedBalanceFilter === 'zero' }]" @click="selectBalanceFilter('zero')" :title="$t('tokenList.balanceZero')">
									...
									<div class="stat-content">
										<div class="stat-label">{{ $t('tokenList.balanceZero') }}</div>
										<div class="stat-value">{{ balanceStatistics.ZERO }}</div>
									</div>
								</div>

								<!-- 无余额数据 -->
								<div :class="['balance-stat-card', 'nodata', { selected: selectedBalanceFilter === 'nodata' }]" @click="selectBalanceFilter('nodata')" :title="$t('tokenList.balanceNodata')">
									...
									<div class="stat-content">
										<div class="stat-label">{{ $t('tokenList.balanceNodata') }}</div>
										<div class="stat-value">{{ balanceStatistics.NODATA }}</div>
									</div>
								</div>
							</div>
						</div>
						...
						<div class="list-header">
							<div class="list-toolbar">
								...
								<div class="search-box">
									...
									<input type="text" v-model="searchQuery" :placeholder="$t('tokenList.searchPlaceholder')" class="search-input" />
								...
								<div class="email-suffix-dropdown">
									<button class="email-suffix-btn" @click.stop="toggleEmailSuffixMenu" :title="selectedEmailSuffix ? `筛选: ${selectedEmailSuffix}` : '按邮箱后缀筛选'" :class="{ active: selectedEmailSuffix }">
										<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
									...
									<Transition name="dropdown">
										<div v-if="showEmailSuffixMenu" class="email-suffix-menu" @click.stop>
											...
											<div class="suffix-menu-header">
												<!-- 全部选项 -->
												<button :class="['suffix-option', 'suffix-option-all', { active: selectedEmailSuffix === null }]" @click="clearEmailSuffixFilter">
													<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
														<path d="M3 13h2v-2H3v2zm0 4h2v-2H3v2zm0-8h2V7H3v2zm4 4h14v-2H7v2zm0 4h14v-2H7v2zM7 7v2h14V7H7z" />
													</svg>
													<span>全部</span>
													<span class="suffix-count">{{ statusFilteredTokens.length }}</span>
													<svg v-if="selectedEmailSuffix === null" width="16" height="16" viewBox="0 0 24 24" fill="currentColor" class="check-icon">
														<path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z" />
													</svg>
												</button>

												<!-- 提取按钮 -->
												<button class="suffix-extract-btn" @click="extractEmailSuffixes" :disabled="emailSuffixes.length === 0" :title="emailSuffixes.length === 0 ? '暂无邮箱后缀可提取' : '复制所有邮箱后缀到剪贴板'">
													<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
											...
											<div v-if="emailSuffixes.length > 0" class="suffix-list">
												<button v-for="suffix in emailSuffixes" :key="suffix" :class="['suffix-option', { active: selectedEmailSuffix === suffix }]" @click="selectEmailSuffix(suffix)">
													<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
														<path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z" />
													</svg>
													<span class="suffix-text">{{ suffix }}</span>
													<span class="suffix-count">{{ emailSuffixStatistics[suffix] }}</span>
													<svg v-if="selectedEmailSuffix === suffix" width="16" height="16" viewBox="0 0 24 24" fill="currentColor" class="check-icon">
														<path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z" />
													</svg>
												</button>
											</div>
		...
		<DatabaseConfig v-if="showDatabaseConfig" @close="showDatabaseConfig = false" @config-saved="handleDatabaseConfigSaved" @config-deleted="handleDatabaseConfigDeleted" />
				...
				<div v-if="showBatchImportDialog" class="batch-import-overlay" @click="showBatchImportDialog = false">
					<div class="batch-import-dialog" @click.stop>
						...
						<div class="batch-import-tabs">
							<button :class="['batch-import-tab', { active: batchImportTab === 'session' }]" @click="batchImportTab = 'session'">
								<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
									<path
							...
							<button :class="['batch-import-tab', { active: batchImportTab === 'token' }]" @click="batchImportTab = 'token'">
								<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
						...
						<div class="dialog-body">
							...
							<div v-if="batchImportTab === 'session'" class="tab-content">
								...
								<div class="session-inputs-container">
									<div v-for="(input, index) in sessionInputs" :key="input.id" class="session-input-item">
										...
										<input v-model="input.value" type="text" :placeholder="$t('tokenList.sessionInputPlaceholder')" class="session-input-field" />
										<button @click="removeSessionInput(input.id)" class="session-input-delete" :title="$t('tokenList.deleteInput')" :disabled="sessionInputs.length <= 1">
											<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
												<path d="M6 19c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7H6v12zM19 4h-3.5l-1-1h-5l-1 1H5v2h14V4z" />
											</svg>
										</button>
								...
								<!-- 添加更多按钮 -->
								<button @click="addSessionInput" @contextmenu="handleContextMenu($event, 'session')" class="add-more-btn">
									<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
										<path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z" />
									</svg>
									{{ $t('tokenList.addMore') }}
							...
							<div v-else-if="batchImportTab === 'token'" class="tab-content">
								...
								<div class="format-option-single">
									...
									<button @click="fillTokenTemplate()" @contextmenu="handleContextMenu($event, 'token')" class="btn-fill-template">
										{{ $t('tokenList.fillTemplate') }}
									</button>
								</div>

								<div class="import-input-section">
									<textarea v-model="importJsonText" rows="10" class="import-textarea" @input="validateImportJson"></textarea>
								</div>
						...
						<div class="dialog-footer">
							...
							<button @click="executeBatchImport" class="btn-confirm" :disabled="isImporting || (batchImportTab === 'session' ? validSessionCount === 0 : importPreview.length === 0)">
								<template v-if="isImporting">
									{{ $t('tokenList.importing') }}
								</template>
								<template v-else>
									{{ batchImportTab === 'session' ? $t('tokenList.batchAdd', { count: validSessionCount }) : $t('tokenList.confirmImport') }}
								</template>
							</button>
						</div>
					</div>
				...
				<div class="context-menu" :style="{ left: contextMenuPosition.x + 'px', top: contextMenuPosition.y + 'px' }" @click.stop>
					...
					<template v-if="contextMenuType === 'session'">
						<div class="context-menu-custom">
							<input v-model.number="customFillCount" type="number" min="1" max="20" :placeholder="$t('tokenList.customCount')" class="custom-count-input" @click.stop />
					...
					<template v-else>
						<div class="context-menu-custom">
							<input v-model.number="customFillCount" type="number" min="1" max="100" :placeholder="$t('tokenList.customCount')" class="custom-count-input" @click.stop />
...
<script setup>
	...
	// 余额筛选管理
	const selectedBalanceFilter = ref('nonzero') // 默认选中"余额>0", null表示"全部", 'zero'表示余额为0, 'nonzero'表示余额>0, 'nodata'表示无数据
	...
	const deletableTokensCount = computed(() => {
		return tokens.value.filter(token => token.ban_status === 'SUSPENDED' || token.ban_status === 'EXPIRED').length
	})
	...
	const filteredTokens = computed(() => {
		...
		// 2. 应用余额筛选
		if (selectedBalanceFilter.value !== null) {
			result = result.filter(token => {
				const balance = token.portal_info?.credits_balance

				if (selectedBalanceFilter.value === 'zero') {
					// 筛选余额为0的
					return balance === 0
				} else if (selectedBalanceFilter.value === 'nonzero') {
					// 筛选余额>0的
					return balance !== undefined && balance !== null && balance > 0
				} else if (selectedBalanceFilter.value === 'nodata') {
					// 筛选无余额数据的
		...
		if (searchQuery.value.trim()) {
			...
			result = result.filter(token => {
				// 原有字段搜索
				const matchesOriginalFields = token.access_token?.toLowerCase().includes(query) || token.email_note?.toLowerCase().includes(query) || token.auth_session?.toLowerCase().includes(query) || token.tag_name?.toLowerCase().includes(query)
	...
	// 选择余额筛选 - 单选模式
	const selectBalanceFilter = filter => {
		// 如果点击当前已选中的筛选,则取消选择(回到"全部")
		if (selectedBalanceFilter.value === filter) {
			selectedBalanceFilter.value = null
		} else {
	...
	const executeBatchImportFromSessionInputs = async () => {
		// 获取所有非空的 session
		const validSessions = sessionInputs.value.map(input => input.value.trim()).filter(value => value.length > 0)
		...
		try {
			...
			if (sessionExtractionErrors.length > 0) {
				const errorDetails = sessionExtractionErrors.map(e => `[${e.index}] ${e.error}`).join('\n')
	...
	const executeBatchDelete = async () => {
		...
		try {
			// 获取要删除的 tokens
			const tokensToDelete = tokens.value.filter(token => token.ban_status === 'SUSPENDED' || token.ban_status === 'EXPIRED')
	...
	const storageStatusText = computed(() => {
		...
		if (isDatabaseAvailable.value) {
			return isSyncNeeded.value ? `${t('storage.dualStorage')}-${t('storage.notSynced')}` : t('storage.dualStorage')
		}
		...
		results.forEach(result => {
			...
			if (token) {
				...
				if ((statusResult.status === 'SUSPENDED' || statusResult.status === 'EXPIRED') && !token.skip_check) {
					...
					hasChanges = true
					// 显示通知
					const autoDisableMsg = statusResult.status === 'SUSPENDED' ? t('messages.autoDisabledBanned') : t('messages.autoDisabledExpired')
	...
	const addToken = tokenData => {
		...
		if (tokenData.emailNote && tokenData.emailNote.trim()) {
			...
			const duplicate = tokens.value.find(token => token.email_note && token.email_note.trim().toLowerCase() === emailToCheck)

			if (duplicate) {
				window.$notify.warning(t('messages.duplicateEmailSkipped', { email: tokenData.emailNote }))
...
<style scoped>
	...
	/* 余额过滤图标颜色 */
	.balance-stat-card .stat-icon.zero {
		background: linear-gradient(135deg, #ef4444 0%, #dc2626 100%);
		color: white;
	}

	.balance-stat-card .stat-icon.nonzero {
		background: linear-gradient(135deg, #10b981 0%, #059669 100%);
		color: white;
	}

	.balance-stat-card .stat-icon.nodata {
		background: linear-gradient(135deg, #6b7280 0%, #4b5563 100%);
		color: white;
	}
	...
	/* 余额过滤栏暗色主题 */
	[data-theme='dark'] .balance-stat-card {
		background: rgba(55, 65, 81, 0.5);
		border-color: transparent;
	}

	[data-theme='dark'] .balance-stat-card:hover {
		background: rgba(55, 65, 81, 0.7);
		box-shadow: 0 4px 8px rgba(0, 0, 0, 0.3);
	}

	[data-theme='dark'] .balance-stat-card.selected {
		border-color: var(--color-primary, #3b82f6);
		background: rgba(59, 130, 246, 0.2);
		box-shadow: 0 2px 8px rgba(59, 130, 246, 0.4);
	}
